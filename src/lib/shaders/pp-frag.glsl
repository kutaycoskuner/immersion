...
float detectDepthEdge(vec2 uv, vec2 texel){
    ...
    float centerDepth = linearizeDepth(texture2D(uTexDepth, uv).r);
    float diff = 0.0;
    float d;

    d = linearizeDepth(texture2D(uTexDepth, uv + vec2(texel.x,0.0)).r);
    diff = max(diff, abs(centerDepth - d));

    d = linearizeDepth(texture2D(uTexDepth, uv - vec2(texel.x,0.0)).r);
    diff = max(diff, abs(centerDepth - d));

    d = linearizeDepth(texture2D(uTexDepth, uv + vec2(0.0,texel.y)).r);
    diff = max(diff, abs(centerDepth - d));
...
// --------------------------------------------
float detectNormalEdge(vec2 uv, vec2 texel, float centerDepth){
    ...
    for(int i=0;i<4;i++){
        ...
        float sampleDepth = linearizeDepth(texture2D(uTexDepth, uvOffset).r);

        // // Z TEST: ignore pixels that are in front
        // if((sampleDepth - centerDepth) < 0.02)
        //     continue;
...
void main() {
    ...
    float centerDepth = linearizeDepth(texture2D(uTexDepth, vUv).r);

    float depthEdge  = detectDepthEdge(vUv, texel);
    float normalEdge = detectNormalEdge(vUv, texel, centerDepth);
    ...
    depthEdge  = clamp(depthEdge  * uOutlineStrength, 0.0, .6);
    ...
    // vec3 finalColor = mix(vec3(0.1).rgb, edgeColor, edge);
    // gl_FragColor = vec4(finalColor, 1.0);


    // final color
    // Blend outline with original color
    vec3 finalColor = mix(color.rgb, outlineColor, depthEdge);
    gl_FragColor = vec4(finalColor, 1.0);
}